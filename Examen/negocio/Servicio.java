...
public class Servicio
{
  ...
  public Response alta_usuario(@FormParam("usuario") Usuario usuario) throws Exception
  {
  ...
  @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
  @Produces(MediaType.APPLICATION_JSON)
  public Response consulta_usuario(@FormParam("email") String email) throws Exception
  {
    Connection conexion= pool.getConnection();
  ...
  @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
  @Produces(MediaType.APPLICATION_JSON)
  public Response modifica_usuario(@FormParam("usuario") Usuario usuario) throws Exception
  {
    Connection conexion= pool.getConnection();
  ...
  public Response borrar_usuarios() throws Exception{
      Connection conexion= pool.getConnection();
      try{
          PreparedStatement stmt_1 = conexion.prepareStatement("SELECT * FROM usuarios;");
          try{
              //stmt_1.setString(1,email);
              ResultSet rs = stmt_1.executeQuery();
              try{
                  if (!rs.next())
                      return Response.status(400).entity(j.toJson(new Error("No existen datos, base de datos vacia"))).build();
              }
              finally{
                  rs.close();
              }
